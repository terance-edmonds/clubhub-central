...
<div id="event-dashboard-event-add"
            ...
            <form class="form" method="post">
                    ...
                    <div class="form-section-content">
                        <div class="multi-wrap">
                            ...
                            <div class="input-wrap">
                                ...
                                <input value="<?= setValue('venue') ?>" id="venue" type="text" name="venue"
                        ...
                        <div class="multi-wrap">
                            <div class="input-wrap">
                                ...
                                <input set-default="datetime" value="<?= setValue('start_datetime') ?>"
                                    id="start_datetime" type="datetime-local" name="start_datetime"
                            ...
                            <div class="input-wrap">
                                ...
                                <input set-default="datetime" value="<?= setValue('end_datetime') ?>" id="end_datetime"
                                    type="datetime-local" name="end_datetime" placeholder="End Date & Time" required>
                ...
                <div class="form-section groups">
                    <div id="event-groups-section" class="form-section-title-wrap">
                        ...
                        <div onclick="onAddNewGroup()" class="button" data-variant="outlined" data-type="icon"
                            data-size="small">
                            <span>Add New</span>
                            ...
                            <div id="<?= $key ?>" class="form-section-content group-form-section">
                                <div class="multi-wrap">
                                    <div class="input-wrap">
                                        ...
                                        <input value="<?= setValue("groups[$key][name]") ?>" id="<?= $key ?>-name" type="text"
                                            name="groups[<?= $key ?>][name]" placeholder="Group Name" required>
                                    ...
                                    <div class="input-wrap">
                                        ...
                                        <select onchange="onAddGroupMember(event, '<?= $key ?>')" value=""
                                            name="group_member_select-<?= $key ?>" id="group_member_select-<?= $key ?>">
                                        ...
                                        <div class="checkbox-wrap">
                                            <label class="checkbox-label">
                                                ...
                                                <input hidden type="checkbox" checked
                                                    name="groups[<?= $key ?>][members][<?= $member['id'] ?>][id]"
                                                    value="<?= $member['id'] ?>">
                                                <input hidden type="checkbox" checked
                                                    name="groups[<?= $key ?>][members][<?= $member['id'] ?>][user_id]"
                                                    value="<?= $member['user_id'] ?>">
                                                <input hidden type="checkbox" checked
                                                    name="groups[<?= $key ?>][members][<?= $member['id'] ?>][name]"
                                                    value="<?= $member['name'] ?>">
                                            </label>
                                ...
                                <div class="sub-section">
                                    ...
                                    <div class="checkboxes-wrap">
                                        <label class="checkbox-label">
                                            <input <?php if (!empty($group['permissions']['budget_permission'])) { ?> checked
                                                <?php } ?> type="checkbox"
                                                name="groups[<?= $key ?>][permissions][budget_permission]" value="1">
                                            Budgets
                                        </label>
                                        <label class="checkbox-label">
                                            <input <?php if (!empty($group['permissions']['details_permission'])) { ?> checked
                                                <?php } ?> type="checkbox"
                                                name="groups[<?= $key ?>][permissions][details_permission]" value="1">
                                            Event Details
                                        </label>
                                        <label class="checkbox-label">
                                            <input <?php if (!empty($group['permissions']['registration_permission'])) { ?>
                                                    checked <?php } ?> type="checkbox"
                                                name="groups[<?= $key ?>][permissions][registration_permission]" value="1">
                                            Registration
                                        </label>
                                        <label class="checkbox-label">
                                            <input <?php if (!empty($group['permissions']['sponsor_permission'])) { ?> checked
                                                <?php } ?> type="checkbox"
                                                name="groups[<?= $key ?>][permissions][sponsor_permission]" value="1">
                                            Sponsors
                                        </label>
                                    </div>
                                </div>
                                ...
                                <div class="form-section-buttons-wrap">
                                    <button type="button" onclick="onRemoveGroup('<?= $key ?>')"
                                        class="icon-button cl-red remove-group-btn">
                                        <span class="material-icons-outlined">
                                            delete
                                        </span>
                                    </button>
                ...
                <div class="buttons-wrap">
                    <button type="submit" name="submit" value="create_event" class="button contained">Create
                        Event</button>
                </div>
            </form>
        </div>
    ...
    <div id="{{group_name}}" class="form-section-content group-template group-form-section">
        <div class="multi-wrap">
            <div class="input-wrap">
                ...
                <input value="<?= setValue("groups[{{group_name}}][name]") ?>" id="{{group_name}}-name" type="text"
                    name="groups[{{group_name}}][name]" placeholder="Group Name" required>
            ...
            <div class="input-wrap">
                ...
                <select onchange="onAddGroupMember(event, '{{group_name}}')" value=""
                    name="group_member_select-{{group_name}}" id="group_member_select-{{group_name}}">
                    <option value="" selected disabled hidden>Choose Member</option>
                    <?php foreach ($club_members_data as $club_member) { ?>
                        <option value="<?= $club_member->id ?>,<?= $club_member->user_id ?>">
                            <?= $club_member->first_name ?>
        ...
        <div class="sub-section">
            ...
            <div class="checkboxes-wrap">
                ...
                <label class="checkbox-label">
                    <input type="checkbox" name="groups[{{group_name}}][permissions][registration_permission]"
                        value="true">
                    Registration
                </label>
                <label class="checkbox-label">
    ...
    <div class="checkbox-wrap group-member-template">
        <label class="checkbox-label">
            {{group_member_name}}
            <input hidden type="checkbox" checked name="groups[{{group_name}}][members][{{group_member_id}}][id]"
                value="{{group_member_id}}">
            <input hidden type="checkbox" checked name="groups[{{group_name}}][members][{{group_member_id}}][user_id]"
                value="{{group_member_user_id}}">
            <input hidden type="checkbox" checked name="groups[{{group_name}}][members][{{group_member_id}}][name]"
                value="{{group_member_name}}">
        </label>
        <span onclick="onRemoveGroupMember(event)" class="material-icons-outlined">
            clear
        </span>
